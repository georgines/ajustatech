        ...
        <div class="card h-100">
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <select class="form-select" wire:model="openingPaymentMethodType">
                        <option value="">{{ trans('financial::messages.select_option') }}</option>
                        @foreach ($paymentMethodTypes as $type)
                            <option value="{{ $type }}">{{ $type }}</option>
                        @endforeach
                    </select>
                    @error('openingPaymentMethodType') <small class="text-danger">{{ $message }}</small> @enderror
                </div>
                <div class="mb-3">
        ...
        <div class="card h-100">
            ...
            <div class="card-body">
                @if ($currentSession)
                    ...
                    <div class="mb-3">
                        <label class="form-label">{{ trans('financial::messages.payment_method_type') }}</label>
                        <select class="form-select" wire:model="closingPaymentMethodType">
                            <option value="">{{ trans('financial::messages.select_option') }}</option>
                            @foreach ($paymentMethodTypes as $type)
                                <option value="{{ $type }}">{{ $type }}</option>
                            @endforeach
                        </select>
                        @error('closingPaymentMethodType') <small class="text-danger">{{ $message }}</small> @enderror
                    </div>
                    <div class="mb-3">
